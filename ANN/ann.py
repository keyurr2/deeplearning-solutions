...
# Initialising the the ANN
classifier = Sequential()

# Adding the input layer and first hidden layer
classifier.add(Dense(20, input_shape=(39,), activation='relu', kernel_initializer='uniform'))

# Adding the hidden layer with nodes, no need for input shape
classifier.add(Dense(20, activation='relu', kernel_initializer='uniform'))

# Adding the output layer with single node as there is binary result
classifier.add(Dense(1, activation='sigmoid', kernel_initializer='uniform'))

# Compiling the ANN
classifier.compile(optimizer = 'adam', loss = 'binary_crossentropy', metrics = ['accuracy'])
# Note : For more than two category (problem other than binary), use softmax activation

# Fitting the ANN to to training set
